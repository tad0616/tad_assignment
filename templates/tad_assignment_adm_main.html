    ...
    <tbody>
      ...
      <{foreach from=$all_data item=as}>
        <tr>
          ...
          <td>
            <a href="javascript:delete_tad_assignment_func(<{$as.assn}>);" class="btn btn-xs btn-danger"><{$smarty.const._TAD_DEL}></a>
            <a href="add.php?op=tad_assignment_form&assn=<{$as.assn}>" class="btn btn-xs btn-warning"><{$smarty.const._TAD_EDIT}></a>
          </td>
        </tr>
      <{/foreach}>
